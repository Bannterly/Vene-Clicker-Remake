...
namespace VeneClicker
{
    public partial class MainForm : Form
    {
            ...
            (GetAsyncKeyState(VK_RBUTTON) & 0x8000) != 0
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    if (isClickerEnabled && (!onlyInMinecraft || IsMinecraftActive()))
                    {
                        ...
                        double maxCpsForButton = i == 0 ? maxCps : rightMaxCps;
                        double minCpsForButton = i == 0 ? minCps : rightMinCps;
                        double targetCps = cpsDrops ? random.NextDouble() * (maxCpsForButton - minCpsForButton) + minCpsForButton : maxCpsForButton;

                        if (cpsDrops)
                        {
                            targetCps = PerformCpsDrop(targetCps, random);
                        }

                        double clickInterval = 1000.0 / targetCps;

                        long currentTime = stopwatches[i].ElapsedMilliseconds;
                        if (currentTime - lastClickTimes[i] >= clickInterval)
                        {
                            if (i == 0)
                            {
        ...
        private double PerformCpsDrop(double targetCps, Random random)
        {
            if (random.NextDouble() < 0.15) // 15% chance of CPS drop
            {
                return 2.0; // Drop to 2 CPS
            }
            return targetCps;
        }
